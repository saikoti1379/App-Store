...
class AppStore extends Component {
  state = {userAppsList: AppsList, tempAppList: AppsList}

  selection = tabId => {
    const {userAppsList} = this.state
    // console.log(tabId)
    // console.log(userAppsList)

    const categoryList = userAppsList.filter(
      appItem => appItem.category === tabId,
    )
    // console.log(categoryList)

    this.setState({tempAppList: categoryList})
  }

  render() {
    const {tempAppList} = this.state
            ...
            <div>
              <ul>
                {TabsList.map(category => (
                  <TabItem category={category} selection={this.selection} />
                ))}
              </ul>
            </div>
            <div>
              <ul>
                {tempAppList.map(eachAppItem => (
                  <AppItem appItem={eachAppItem} key={eachAppItem.appId} />
                ))}
              </ul>
            </div>
          </div>
